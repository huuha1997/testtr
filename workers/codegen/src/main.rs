...
const STREAM_KEY: &str = "q.codegen";
const NEXT_STREAM_KEY: &str = "q.ci";
const DLQ_STREAM_KEY: &str = "q.codegen.dlq";
const GROUP: &str = "cg.codegen";
...
async fn main() -> anyhow::Result<()> {
    ...
    let consumer_name = std::env::var("CODEGEN_CONSUMER_NAME")
        .unwrap_or_else(|_| format!("codegen-worker-{}", Uuid::new_v4()));
    ...
    info!(consumer = %consumer_name, "codegen worker started");
...
async fn handle_message(
    ...
    match process_job(db, state, &job, conn).await {
        ...
        Err(err) => {
            warn!(error = %err, run_id = %job.run_id, "codegen processing failed");
...
async fn process_job(
    ...
    let repair_iteration = input_payload
        .get("repair_iteration")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let force_fail_gates = input_payload.get("force_fail_gates").cloned();

    let output = call_gateway_codegen(state, job.run_id, input_payload).await?;

    sqlx::query("UPDATE runs SET status = $2 WHERE id = $1")
        .bind(job.run_id)
        .bind(RunStatus::CodegenRunning.as_str())
        .execute(db)
        .await?;
    sqlx::query(
    ...
    enqueue(
        ...
        &QueueJob {
            ...
            payload_json: serde_json::json!({
                "source": "codegen_worker",
                "repair_iteration": repair_iteration,
                "force_fail_gates": force_fail_gates,
    ...
    .await?;
    info!(run_id = %job.run_id, "codegen job processed");
    Ok(())
}

async fn call_gateway_codegen(
    state: &WorkerState,
    run_id: Uuid,
    input_payload: serde_json::Value,
) -> anyhow::Result<serde_json::Value> {
    let url = format!("{}/mcp/codegen/run", state.mcp_gateway_base_url);
    ...
    if !resp.status().is_success() {
        ...
        return Err(anyhow::anyhow!(
            "mcp-gateway codegen failed: http {}: {}",
            status.as_u16(),
            body
        ));
    }
    let out: GatewayResponse = resp
        .json()
        .await
        .context("invalid mcp-gateway codegen response")?;
    Ok(out.raw)
}
